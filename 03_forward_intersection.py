...
# 计算一个同名点的模型坐标
def intersect_one_point(p, r_left, r_right):
    baseline = 1.0
    ...
    p_left = n1 * left_ray
    p_right = np.array([baseline, 0.0, 0.0]) + n2 * right_ray
    ...
    result["Q"] = gap[1]
    result["ray_gap"] = np.linalg.norm(gap)
    return result

# 所有点前方交会
def forward_points(points, u):
...
def main():
    ...
    control_fields = ["id", "number", "model_x", "model_y", "model_z", "Q", "ray_gap", "ground_x", "ground_y", "ground_z"]
    object_fields = ["id", "object", "number", "model_x", "model_y", "model_z", "Q", "ray_gap"]
    ...
    print("前方交会完成")
    print("像控点平均光线间距 =", control_gap.mean())
    print("地物点平均光线间距 =", object_gap.mean())
